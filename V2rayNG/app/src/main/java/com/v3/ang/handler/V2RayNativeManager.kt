 ...
 *
 * Thread-safe singleton wrapper for Libv3 native methods.
...
object V2RayNativeManager {
    ...
    fun initCoreEnv(context: Context?) {
        if (initialized.compareAndSet(false, true)) {
            try {
                ...
                Libv3.initCoreEnv(assetPath, deviceId)
    ...
    fun measureOutboundDelay(config: String, testUrl: String): Long {
        return try {
            Libv3.measureOutboundDelay(config, testUrl)
        } catch (e: Exception) {
    ...
    fun newCoreController(handler: CoreCallbackHandler): CoreController {
        return try {
            Libv3.newCoreController(handler)
        } catch (e: Exception) {
